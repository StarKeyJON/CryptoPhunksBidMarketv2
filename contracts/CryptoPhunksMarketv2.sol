...
contract CryptoPhunksMarket is ReentrancyGuard, Pausable, Ownable {
    ...
    function phunkNoLongerForSale(uint[] calldata phunkIndex) public nonReentrant() {
        for(uint i; i<phunkIndex.length; i++){
            if (phunkIndex[i] >= 10000) revert('token index not valid');
            if (phunksContract.ownerOf(phunkIndex[i]) != msg.sender) revert('you are not the owner of this token');
            phunksOfferedForSale[phunkIndex[i]] = Offer(false, phunkIndex[i], msg.sender, 0, address(0x0));
            emit PhunkNoLongerForSale(phunkIndex[i]);
        } 
    }

    /* Allows a CryptoPhunk owner to offer it for sale */
    function offerPhunkForSale(uint[] calldata phunkIndex, uint[] calldata minSalePriceInWei) public whenNotPaused nonReentrant()  {
        for(uint i; i<phunkIndex.length; i++){
            if (phunkIndex[i] >= 10000) revert('token index not valid');
            if (phunksContract.ownerOf(phunkIndex[i]) != msg.sender) revert('you are not the owner of this token');
            phunksOfferedForSale[phunkIndex[i]] = Offer(true, phunkIndex[i], msg.sender, minSalePriceInWei[i], address(0x0));
            emit PhunkOffered(phunkIndex[i], minSalePriceInWei[i], address(0x0));
        }
    }

    /* Allows a CryptoPhunk owner to offer it for sale to a specific address */
    function offerPhunkForSaleToAddress(uint[] calldata phunkIndex, uint[] calldata minSalePriceInWei, address[] calldata toAddress) public whenNotPaused nonReentrant() {
        for(uint i; i<phunkIndex.length; i++){
            if (phunkIndex[i] >= 10000) revert();
            if (phunksContract.ownerOf(phunkIndex[i]) != msg.sender) revert('you are not the owner of this token');
            phunksOfferedForSale[phunkIndex[i]] = Offer(true, phunkIndex[i], msg.sender, minSalePriceInWei[i], toAddress[i]);
            emit PhunkOffered(phunkIndex[i], minSalePriceInWei[i], toAddress[i]);
        }
    }
    

    /* Allows users to buy a CryptoPhunk offered for sale */
    function buyPhunk(uint[] calldata phunkIndex) payable public whenNotPaused nonReentrant() {
        uint totalValue;
        for(uint i; i<phunkIndex.length; i++){
            if (phunkIndex[i] >= 10000) revert('token index not valid');
            Offer memory offer = phunksOfferedForSale[phunkIndex[i]];
            ...
            /// Checking msg.value after iteration
            address seller = offer.seller;
            if (seller == msg.sender) revert('seller == msg.sender');
            if (seller != phunksContract.ownerOf(phunkIndex[i])) revert('seller no longer owner of phunk'); // Seller no longer owner of phunk
            totalValue += offer.minValue;
            phunksOfferedForSale[phunkIndex[i]] = Offer(false, phunkIndex[i], msg.sender, 0, address(0x0));
            phunksContract.safeTransferFrom(seller, msg.sender, phunkIndex[i]);
            pendingWithdrawals[seller] += msg.value;
            emit PhunkBought(phunkIndex[i], msg.value, seller, msg.sender);

            // Check for the case where there is a bid from the new owner and refund it.
            // Any other bid can stay in place.
            Bid memory bid = phunkBids[phunkIndex[i]];
            if (bid.bidder == msg.sender) {
                // Kill bid and refund value
                pendingWithdrawals[msg.sender] += bid.value;
                phunkBids[phunkIndex[i]] = Bid(false, phunkIndex[i], address(0x0), 0);
            }
        }
        if (msg.value != totalValue) revert('not enough ether');          // Didn't send enough ETH
    }
    ...
    function acceptBidForPhunk(uint[] calldata phunkIndex, uint[] calldata minPrice) public whenNotPaused nonReentrant() {
        for(uint i; i<phunkIndex.length; i++){
            if (phunkIndex[i] >= 10000) revert('token index not valid');
            if (phunksContract.ownerOf(phunkIndex[i]) != msg.sender) revert('you do not own this token');
            address seller = msg.sender;
            Bid memory bid = phunkBids[phunkIndex[i]];
            if (bid.value == 0) revert('cannot enter bid of zero');
            if (bid.value < minPrice[i]) revert('your bid is too low');

            address bidder = bid.bidder;
            if (seller == bidder) revert('you already own this token');
            phunksOfferedForSale[phunkIndex[i]] = Offer(false, phunkIndex[i], bidder, 0, address(0x0));
            uint amount = bid.value;
            phunkBids[phunkIndex[i]] = Bid(false, phunkIndex[i], address(0x0), 0);
            phunksContract.safeTransferFrom(msg.sender, bidder, phunkIndex[i]);
            pendingWithdrawals[seller] += amount;
            emit PhunkBought(phunkIndex[i], bid.value, seller, bidder);
        }
    }
